...
import './contrast.css';
import './polish.css';
import { configureDatabase, deleteDraft, getDrafts, resetDemoDatabase, saveDraft } from './db';
import { jpegCapturedAt } from './exif';
import { importedDraft, jsonBlob, pdfBlob } from './export';
import { bytesLabel, exportIssues, localDateTime, precisionLabels, shareSafeText, sharedCoordinates } from './privacy';
import { newDraft, type Candidate, type Confidence, type Precision, type SightingDraft } from './types';
import { attachmentBudgetIssue, MAX_FILE_BYTES, MAX_FILES, MAX_IMPORT_BYTES } from './limits';

const app = document.querySelector<HTMLDivElement>('#app')!;
const url = new URL(location.href);
const isDemo = url.pathname.replace(/\/$/, '') === '/demo' || url.searchParams.get('demo') === '1';
configureDatabase(isDemo);
document.body.classList.toggle('demo-mode', isDemo);
if (isDemo) {
  document.title = 'Demo — Bird Sighting Proof Card';
  document.querySelector<HTMLLinkElement>('link[rel="canonical"]')?.setAttribute('href', 'https://bird-sighting-proof-card.sociobot.in/demo/');
  document.querySelector<HTMLMetaElement>('meta[name="description"]')?.setAttribute('content', 'Try a filled bird-sighting record with isolated sample data.');
  document.querySelectorAll<HTMLMetaElement>('meta[property="og:title"],meta[name="twitter:title"]').forEach(meta => meta.content = 'Demo — Bird Sighting Proof Card');
  document.querySelectorAll<HTMLMetaElement>('meta[property="og:description"],meta[name="twitter:description"]').forEach(meta => meta.content = 'Try a filled bird-sighting record with isolated sample data.');
  document.querySelector<HTMLMetaElement>('meta[property="og:url"]')?.setAttribute('content', 'https://bird-sighting-proof-card.sociobot.in/demo/');
}
...
app.innerHTML = `
  <header class="site-head">
    ...
    <nav aria-label="Main navigation"><a href="/demo/">Demo</a><a href="/#builder">Make a record</a><a href="/privacy/">Privacy</a></nav>
  </header>
  ${isDemo ? '<aside class="demo-banner" aria-label="Demo mode"><strong>Demo — sample data, nothing is saved to your records.</strong><div><button type="button" id="reset-demo">Reset demo</button><button type="button" id="start-real">Start for real</button></div></aside>' : ''}
  <main id="main">
    <section class="hero" aria-labelledby="page-title">
      <div class="hero-copy"><p class="eyebrow">Private bird record · no account</p><h1 id="page-title" tabindex="-1">${isDemo ? 'Review a sample bird record' : 'Make a private bird-sighting record'}</h1><p class="lede">For birders who need evidence others can review without sharing an exact nest location.</p><div class="hero-actions"><a class="button primary" href="/demo/">Try it with sample data</a><span>Opens a filled record you can export.</span><a class="button secondary" href="/#builder">Start a blank record</a></div><ul class="hero-facts"><li>Free to use.</li><li>Works offline after your first visit.</li><li>Your evidence stays on this device until you export.</li></ul></div>
      <figure class="hero-plate"><picture><source type="image/avif" srcset="/assets/hero-field-map-768.avif 768w, /assets/hero-field-map-1280.avif 1280w" sizes="(max-width: 760px) 100vw, 52vw"><source type="image/webp" srcset="/assets/hero-field-map-768.webp 768w, /assets/hero-field-map-1280.webp 1280w" sizes="(max-width: 760px) 100vw, 52vw"><img src="/assets/hero-field-map-1280.webp" alt="A distant shorebird in a misty marsh crossed by faint topographic contour lines" width="1280" height="853" fetchpriority="high" decoding="async"></picture><figcaption><span>Field illustration</span> Put certainty in your notes, not the picture.</figcaption></figure>
    </section>
    <div class="trust-strip" aria-label="Privacy and connection status"><span class="status-dot" aria-hidden="true"></span><strong id="connection-label">Ready offline</strong><span>Photos, audio, and saved records stay in this browser.</span><button class="link-button" id="storage-help" type="button">How storage works</button></div>
    <section class="builder-intro" id="builder"><div><p class="eyebrow">Make a bird record</p><h2>Record what another birder needs.</h2></div><p>Add a time, broad place, evidence file, and possible species. You can save an incomplete record.</p></section>
    <div id="error-summary" class="error-summary" role="alert" tabindex="-1" hidden></div>
    <div class="workbench">
      <form id="proof-form" novalidate>
        <section class="waypoint" aria-labelledby="evidence-title"><div class="waypoint-mark" aria-hidden="true">01</div><div class="waypoint-body"><p class="overline">Evidence</p><h2 id="evidence-title">Add what you captured</h2><p class="hint">Files stay on this device. JSON exports remove known location metadata from supported files.</p><label class="file-drop" for="evidence-files"><strong>Add photo or audio</strong><span>JPEG, PNG, WebP, MP3, or WAV · 12 MB total</span></label><input class="visually-hidden" id="evidence-files" type="file" accept="image/jpeg,image/png,image/webp,audio/mpeg,audio/wav,audio/x-wav" multiple><div id="file-error" class="field-error" role="alert"></div><ul id="evidence-list" class="evidence-list"></ul></div></section>
        ...
        <section class="waypoint" aria-labelledby="finish-title"><div class="waypoint-mark" aria-hidden="true">06</div><div class="waypoint-body"><p class="overline">Save or export this record</p><h2 id="finish-title">Name this bird record</h2><div class="field"><label for="card-title">Record title <span class="optional">optional</span></label><input id="card-title" maxlength="90" placeholder="e.g. Distant seabird at Deerness"></div><div class="form-actions"><button type="button" class="button primary" id="save-card">Save on this device</button><button type="button" class="button text" id="new-card">Start a new record</button></div><p class="save-status" id="save-status" aria-live="polite">Not saved yet.</p></div></section>
      </form>
  ...
  <footer><div><img src="/icon.svg" alt="" width="32" height="32"><p><strong>Bird Sighting Proof Card</strong><br>Make a reviewable record without publishing it.</p></div><nav aria-label="Footer"><a href="/privacy/">Privacy</a><a href="/terms/">Terms</a><a href="https://github.com/B-Divyesh/sf-bird-sighting-proof-card" rel="external">View source on GitHub (external)</a></nav><p class="art-credit">Built by Param Factory · Build 2026.08.28-p1<br>Original field illustration created with AI assistance.</p></footer>`;
...
const sampleWav = () => {
  ...
  for (let index = 0; index < samples; index++) view.setInt16(44 + index * 2, Math.sin(index / 9) * 2600, true);
  return new Blob([buffer], { type: 'audio/wav' });
};

async function sampleDraft() {
  const sample = newDraft();
  const image = await fetch('/assets/hero-field-map-768.webp').then(response => response.blob());
  const audio = sampleWav();
  sample.title = 'Distant wader at Deerness';
  sample.observedAt = '2026-08-24T06:42';
  sample.timeSource = 'photo metadata';
  sample.placeLabel = 'Deerness coast, Orkney';
  sample.latitude = 58.951234;
  sample.longitude = -2.751234;
  sample.precision = '10km';
  sample.sensitive = true;
  sample.fieldMarks = ['Size & shape', 'Bill', 'Plumage', 'Flight', 'Habitat'];
  sample.fieldNotes = 'Small wader on the outer mud edge. Pale belly, dark legs, and a short straight bill. Brief low flight over the inlet.';
  sample.candidates = [
    { id: crypto.randomUUID(), name: 'Dunlin', confidence: 'medium', notes: 'Size and bill fit. Plumage was hard to judge in dawn light.' },
    { id: crypto.randomUUID(), name: 'Sanderling', confidence: 'low', notes: 'Paler than expected, but the bill looked too long.' }
  ];
  sample.attachments = [
    { id: crypto.randomUUID(), name: 'dawn-marsh-observation.webp', type: 'image/webp', size: image.size, lastModified: Date.parse('2026-08-24T06:42:00Z'), capturedAt: '2026-08-24T06:42', blob: image },
    { id: crypto.randomUUID(), name: 'shore-call-note.wav', type: 'audio/wav', size: audio.size, lastModified: Date.parse('2026-08-24T06:43:00Z'), capturedAt: '2026-08-24T06:43', blob: audio }
  ];
  return sample;
}
...
async function persist(explicit: boolean) {
  ...
  try {
    ...
    if (explicit) announce(isDemo ? 'Demo record saved only in demo storage.' : 'Bird record saved on this device.');
  } catch {
...
function renderPreview() {
  ...
  byId('preview-content').innerHTML = `<dl class="proof-facts"><div><dt>Observed</dt><dd>${escapeHtml(localDateTime(draft.observedAt))}</dd></div><div><dt>Place shared</dt><dd>${escapeHtml(shareSafeText(draft, draft.placeLabel || 'Not recorded'))}</dd></div><div><dt>Coordinates</dt><dd>${shared ? `<span class="coordinates">${shared.latitude}, ${shared.longitude}</span>` : 'Withheld'}<small>${escapeHtml(precisionLabels[draft.precision])}</small></dd></div></dl>${draft.sensitive ? '<p class="sensitive-flag"><strong>Sensitive location</strong> Share only with trusted reviewers.</p>' : ''}<section class="preview-section"><h3>Field marks</h3><p>${draft.fieldMarks.length ? draft.fieldMarks.map(mark => escapeHtml(shareSafeText(draft, mark))).join(' · ') : 'None recorded yet.'}</p><p>${escapeHtml(shareSafeText(draft, draft.fieldNotes || 'No observation notes yet.'))}</p></section><section class="preview-section"><h3>Candidates</h3>${candidates.length ? `<ol>${candidates.map(candidate => `<li><strong>${escapeHtml(shareSafeText(draft, candidate.name))}</strong><span>${escapeHtml(candidate.confidence)} confidence</span>${candidate.notes ? `<small>${escapeHtml(shareSafeText(draft, candidate.notes))}</small>` : ''}</li>`).join('')}</ol>` : '<p>No candidates added yet.</p>'}</section><section class="preview-section"><h3>Evidence</h3><p>${draft.attachments.length ? draft.attachments.map(file => escapeHtml(shareSafeText(draft, file.name))).join(' · ') : 'No files attached yet.'}</p></section><p class="disclaimer">This record supports review. It is not an authoritative identification.</p>`;
}

function renderDrafts() {
  const list = byId('draft-list');
  const ordered = [...drafts].sort((a,b) => b.updatedAt.localeCompare(a.updatedAt));
  if (!ordered.length) { list.innerHTML = '<div class="empty-state"><span aria-hidden="true">⌁</span><h3>No saved records yet</h3><p>A record appears here after you save it on this device.</p><a href="#builder" class="button secondary">Return to the record</a></div>'; return; }
  ...
  list.querySelectorAll<HTMLButtonElement>('[data-open]').forEach(button => button.addEventListener('click', () => {
    const found = drafts.find(item => item.id === button.dataset.open); if (!found) return; draft = found; populateForm(); renderDrafts(); document.querySelector('#builder')?.scrollIntoView({ behavior: 'smooth' }); announce('Saved record opened.');
  }));
  list.querySelectorAll<HTMLButtonElement>('[data-delete]').forEach(button => button.addEventListener('click', async () => {
    ...
    await deleteDraft(found.id); drafts = await getDrafts(); if (draft.id === found.id) { draft = newDraft(); populateForm(); } renderDrafts(); announce('Saved record deleted.');
  }));
}
...
function download(blob: Blob, extension: string) {
  ...
  link.href = url; link.download = `${name || 'bird-sighting-record'}.${extension}`; link.click(); window.setTimeout(() => URL.revokeObjectURL(url), 1000);
}
...
input('evidence-files').addEventListener('change', async event => {
  ...
  for (const file of files) {
    if (!/^(image\/(jpeg|png|webp)|audio\/(mpeg|wav|x-wav))$/.test(file.type)) { error.textContent = `${file.name} is not a supported photo or audio format. Use JPEG, PNG, WebP, MP3, or WAV.`; continue; }
    if (file.size > MAX_FILE_BYTES) { error.textContent = `${file.name} is over the 12 MB file limit.`; continue; }
    if (draft.attachments.length >= MAX_FILES) { error.textContent = `A record can hold up to ${MAX_FILES} evidence files.`; break; }
    const budgetIssue = attachmentBudgetIssue([...draft.attachments.map(item => item.size), file.size]);
    if (budgetIssue) { error.textContent = budgetIssue; continue; }
...
byId('new-card').addEventListener('click', () => { if ((draft.title || draft.placeLabel || draft.attachments.length) && !confirm('Start a new record? Your current changes have been saved on this device.')) return; void persist(false); draft = newDraft(); populateForm(); renderDrafts(); input('place-label').focus(); announce('New blank record started.'); });
...
async function start() {
  try {
    drafts = await getDrafts();
    if (drafts.length) draft = [...drafts].sort((a,b) => b.updatedAt.localeCompare(a.updatedAt))[0];
    else if (isDemo) { draft = await sampleDraft(); await saveDraft(draft); drafts = [draft]; }
  }
  ...
  if (isDemo) requestAnimationFrame(() => document.querySelector('#builder')?.scrollIntoView());
