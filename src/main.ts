...
app.innerHTML = `
  ...
  <main id="main">
    ...
    <dialog id="storage-dialog"><form method="dialog"><button class="dialog-close" aria-label="Close storage explanation">×</button><p class="eyebrow">Local means local</p><h2>Nothing is uploaded.</h2><p>Drafts and evidence live in this browser’s private storage. Clear site data or uninstall the app and they can disappear, so export JSON for a portable backup.</p><p>PDF is a lightweight review sheet. JSON embeds metadata-sanitized image copies and original audio.</p><button class="button primary" value="close">Got it</button></form></dialog>
